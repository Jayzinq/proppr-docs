...
async function run() {
    ...
    const teamDocs = await db.collection("all_positive_team_alerts").find(
        ...
        { projection: { Match: 1, Date: 1, Time: 1, League: 1, Country: 1 }, limit: 5000 }
    ).toArray();

    console.log("Found teamDocs:", teamDocs.length);
    const mvs = teamDocs.find(d => d.Match === "Mexico vs Serbia");
    console.log("Mexico vs Serbia:", mvs);
