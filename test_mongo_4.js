...
async function run() {
    ...
    // How about the other way? Serbia vs Mexico?
    const docs4 = await db.collection("all_positive_alerts").find({ Match: { $regex: /Serbia.*Mexico/i } }).limit(5).toArray();
    console.log("Serbia vs Mexico alerts:", docs4.map(d => `${d.Match} - ${d.Date} - ${d.League}`));
