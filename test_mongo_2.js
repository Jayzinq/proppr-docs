...
async function run() {
    ...
    const docs = await db.collection("all_positive_alerts").find({ Match: { $regex: /Mexico|Serbia/i } }).limit(5).toArray();
    console.log("Alerts:", docs.map(d => `${d.Match} - ${d.Date} - ${d.League}`));
    await client.close();
}
run();
