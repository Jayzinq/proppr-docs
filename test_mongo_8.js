...
async function run() {
    ...
    // Check odds_api_events for ANY match with Mexico and Serbia
    const docs = await db.collection("odds_api_events").find({ $and: [{ $or: [{home: /Mexico/i}, {away: /Mexico/i}] }, { $or: [{home: /Serbia/i}, {away: /Serbia/i}] }] }).limit(5).toArray();
